...
async fn shell_ws_handler(socket: WebSocket, srv: ServerState) {
    ...
    // Inbound: WebSocket frames. Binary = raw keystrokes (xterm.js's
    // `term.onData` emits Uint8Array bytes). Text = JSON control frames.
    ...
    match kind {
        "resize" => {
            ...
            if cols > 0 && rows > 0 {
                ...
                tracing::debug!("shell WS resize {cols}x{rows}");
            }
            true
        }
        "input" => {
...
async fn post_shell_resize(
    ...
    srv.shell.ensure_session();
    let cols = body.get("cols").and_then(|x| x.as_u64()).unwrap_or(0) as u16;
    let rows = body.get("rows").and_then(|x| x.as_u64()).unwrap_or(0) as u16;
    if cols == 0 || rows == 0 {
        return (StatusCode::BAD_REQUEST, "cols and rows must be > 0").into_response();
    }
    srv.shell.resize(cols, rows);
