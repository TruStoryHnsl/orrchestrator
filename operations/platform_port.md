...
13 | Repository Manager | skill:merge-to-main | MANDATORY — merge the session branch back to main so follow-up work (patches, next release cycle) starts from the published state; abort and escalate on cross-session conflicts; delete the session branch after successful merge

Interrupts: target platform toolchain, credentials, or signing identity becomes unavailable mid-port — pause, document blocker, report to user. Platform rejects submission during review — capture the reviewer feedback, reroute to step 4 for remediation. Merge-to-main conflict requires user resolution before the port is considered complete.
