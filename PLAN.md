...
112. [ ] **PI-001: PI harness library entry + BackendKind** — Add `library/harnesses/pi.md` with full capabilities (multi-provider: Anthropic/Google/Mistral, RPC mode, SDK embedding, extensions, skills, custom tools, thinking levels). Add `BackendKind::Pi` to `crates/orrch-core/src/backend.rs`. Auto-detect via `which pi`. Wire into `is_provider_available()` and `send_api_message()`. `BackendConfig` command = `pi`, flags include `--no-session --mode rpc` for pipeline use and bare `pi` for interactive. Update harness loader to surface it in Design > Library > Harnesses.

113. [ ] **PI-002: PI RPC session backend** — New `crates/orrch-core/src/pi_rpc.rs`: `PiRpcSession` struct that spawns `pi --mode rpc --provider <p> --model <m>` as a child process, writes JSONL commands to stdin, reads JSONL events from stdout. Implement `prompt()`, `steer()`, `abort()`, and `get_state()` mapped to PI's protocol. Map PI event types (`agent_start`, `agent_done`, `tool_call`, `tool_result`, `text_delta`) to existing `SessionState` and `OutputSignal` enums so the TUI's Hypervise panel works without changes. `ManagedSession.last_output` populated from structured `text_delta` events — no tmux scraping. Expose as an alternative spawn path in `windows.rs` when backend == Pi.

114. [ ] **PI-003: Skill/tool → PI extension translation** — `x` key on any item in Design > Library > Skills or Design > Library > Tools exports a TypeScript PI extension scaffold to `library/pi-extensions/<name>.ts`. Translation rules: skill `.md` prompt → `getSystemPrompt` return value in extension; tool shell script → `registerTool` execute body calling `bash`; frontmatter `description`/`tags` → extension metadata. Add `PiExtensions` tab to `LibrarySub` enum and `draw_library` — read-only browser of `library/pi-extensions/*.ts` files with syntax-highlighted preview (treat as plain text). `n`=new from template creates a blank extension using `PI_EXTENSION_TEMPLATE` constant in `orrch-library/src/templates.rs`. Extensions are not created through a separate workflow — they are first-class library items alongside skills and tools, buildable and editable via the same `e`=edit-in-vim pattern.
