...
90. [x] **OPT-010: Session lifecycle management** — Unified system: (a) hot/cold project tracking in Oversee ✓; (b) stale session cleanup ✓; (c) session status indicators ✓; (d) session close protocol ✓; (e) session brief navigation in project details TUI ✓.
...
### UI Polish & Settings & Enterprise UX (from Instruction Inbox — 2026-04-22)

_Queued work from `instructions_inbox.md` (OPT-018 through OPT-020). Source idea file: `plans/2026-05-06-03-57.md`. Formally incorporated 2026-04-22._

**Cross-References & Dependencies:**
- **OPT-018 → item 37 (web node editor):** Item 37 ships the `orrch-webedit` crate and `Ctrl+w` launcher. OPT-018 is a display-only fix on the TUI-side portal widget that renders the launched URL — no change to the server/editor itself. Truncate host:port or show icon + port only.
- **OPT-019 ↔ item 59 (API valves), OPT-005 (logo), OPT-013 (lifecycle), TOK-002 (session limits), OPT-011 (editor):** A unified settings modal centralizes toggles currently scattered across panels. Not a replacement for the per-domain UIs (valves in Library > Models stay) — this is a single entry point that exposes all user-configurable options in categorized sections and persists to config.
- **OPT-020 ↔ EVERYTHING:** UX audit + enterprise refactor cross-cuts every panel. Builds on (does not replace) completed UI items — item 66 (focus_depth nav), item 78 (scroll architecture), item 79 (dead-keybind audit), OPT-004 (nav traps), OPT-017 (Hypervise expansion), INS-001 (Plans panel), OPT-019 (settings popup). Audit first, then the `frontend-design` skill drives the refactor. Not a directive to discard existing work — an enterprise-grade polish pass on layout, density, affordances, discoverability, and consistency across all panels.

120. [x] **OPT-018: Truncate webportal URL display** — The Webui/webportal widget currently renders the full URL (`http://127.0.0.1:<port>/...`). Replace with a truncated form: `host:port` only, or an icon + port badge. Remove full-URL rendering from portal list and status widgets. Acceptance: no full URL string shown anywhere in the TUI for launched webportals; clicking/pressing the action key still opens the full URL in the browser. *(Cross-ref: item 37.)* (source: plans/2026-05-06-03-57.md)

121. [ ] **OPT-019: Dedicated settings popup (modal overlay)** — Add a single settings interface accessible from anywhere in the TUI (global keybind, e.g. `,` or `F10`). Modal overlay with categorized sections: Display (themes, color modes, density, tab label width), Webui (access toggles, auto-launch, port range), Editor (nvim flags, terminal emulator preference), Sessions (concurrent limits per-project default, device class override, hot/cold thresholds), Valves (mirror of Library > Models valve state), Tmux (status bar, naming, split behavior), Library sync (repo URL, auto-pull cadence). Persists to `~/.config/orrchestrator/settings.json` (or merges into existing `config.json`). Acceptance: every user-configurable toggle reachable from one place; closing the modal applies changes without restart where possible. *(Cross-ref: item 59, OPT-005, OPT-013, TOK-002, OPT-011.)* (source: plans/2026-05-06-03-57.md)

122. [x] **OPT-020a: TUI UX audit (pre-refactor)** — Thorough UX analysis of the current TUI. Scope: every panel (Design, Oversee, Hypervise, Analyze, Publish) and every sub-view. Evaluate layout hierarchy, information density, affordances (are actions discoverable?), consistency of keybinds and visual language across panels, navigation model coherence (focus_depth, section focus, modal stacks), status/feedback clarity, error states, empty states, loading states, typography/color use within the 256-color constraint. Output: `docs/ux-audit-2026-04-22.md` with per-panel findings, prioritized issue list, and a style guide draft (colors, borders, spacing rules, common widgets). Audit MUST precede OPT-020b. (source: plans/2026-05-06-03-57.md)

123. [ ] **OPT-020b: Enterprise-grade TUI refactor via frontend-design skill** — After OPT-020a lands, invoke the `frontend-design` skill (see `skill-creator:frontend-design`) with the audit findings as input to refactor the entire TUI into an enterprise-grade interface. Scope covers EVERY panel, not a subset — Design/Oversee/Hypervise/Analyze/Publish and all sub-views. Target qualities: distinctive but professional visual language, consistent widget library, clear affordances, dense but legible information display, polished empty/loading/error states, cohesive keybind scheme. Acceptance: every panel renders through the refactored widget library; no legacy ad-hoc layout code remains; the TUI feels like a single coherent product, not a stitched-together prototype. **Depends on OPT-020a** and on the audit-driven style guide. *(Cross-ref: item 66, item 78, item 79, OPT-004, OPT-017, INS-001, OPT-019.)* (source: plans/2026-05-06-03-57.md)

124. [x] **VIS-001: Per-scope project visibility toggle** — Add a `hidden_scopes: HashSet<Scope>` to `Config` (persisted to `~/.config/orrchestrator/config.json`, default empty). Filter `App::projects` and the hot/cold/ignored index buckets by visibility before rendering Oversee. Press `V` in Oversee to open a small modal listing the four scopes (Personal/Private/Public/Commercial); Enter on a scope toggles it. Oversee header shows `(N hidden)` when any scope is hidden. Mirrors the `show_deprecated` UX pattern. Acceptance: hiding `Personal` removes those projects from the list; setting persists across restarts; legacy config.json without the new field still loads. (NOT manually verified in a real terminal yet — needs user observation per CLAUDE.md "look at the result of your work" rule.)
...
113. [x] **PI-002: PI RPC session backend** — New `crates/orrch-core/src/pi_rpc.rs`: `PiRpcSession` struct that spawns `pi --mode rpc --provider <p> --model <m>` as a child process, writes JSONL commands to stdin, reads JSONL events from stdout. Implement `prompt()`, `steer()`, `abort()`, and `get_state()` mapped to PI's protocol. Map PI event types (`agent_start`, `agent_done`, `tool_call`, `tool_result`, `text_delta`) to existing `SessionState` and `OutputSignal` enums so the TUI's Hypervise panel works without changes. `ManagedSession.last_output` populated from structured `text_delta` events — no tmux scraping. Expose as an alternative spawn path in `windows.rs` when backend == Pi.

114. [x] **PI-003: Skill/tool → PI extension translation** — `x` key on any item in Design > Library > Skills or Design > Library > Tools exports a TypeScript PI extension scaffold to `library/pi-extensions/<name>.ts`. Translation rules: skill `.md` prompt → `getSystemPrompt` return value in extension; tool shell script → `registerTool` execute body calling `bash`; frontmatter `description`/`tags` → extension metadata. Add `PiExtensions` tab to `LibrarySub` enum and `draw_library` — read-only browser of `library/pi-extensions/*.ts` files with syntax-highlighted preview (treat as plain text). `n`=new from template creates a blank extension using `PI_EXTENSION_TEMPLATE` constant in `orrch-library/src/templates.rs`. Extensions are not created through a separate workflow — they are first-class library items alongside skills and tools, buildable and editable via the same `e`=edit-in-vim pattern.
...
## Recent Changes
- 2026-04-25: **WebUI dual-page swipe terminal — independent tmux session as Page 2.** `crates/orrch-webui` now serves a horizontal scroll-snap container with two full-viewport panels at `/`. Page 1 (`#panel-mirror`) preserves the existing TUI mirror over `/pty`. Page 2 (`#panel-shell`) attaches an independent xterm.js to a dedicated tmux session (default `orrch-web-shell`, override via `ORRCH_WEBUI_TMUX_SESSION`) over a new `/shell` WebSocket and `/shell/size` JSON endpoint. Bidirectional verified end-to-end with a real WS client: browser keystrokes land in tmux via `send-keys -H`, tmux pane changes broadcast back to subscribers via a 120 ms `capture-pane` poller. Top-bar indicator dots, Page Up/Down + Alt+ArrowLeft/Right keyboard nav, click-to-page nav, and shared mobile gamepad (dispatched to whichever panel is active). New module `crates/orrch-webui/src/shell.rs` (ShellBridge), new `static/js/shell.js`, restructured `static/terminal.html`, integration tests in `tests/swipe_terminal.rs`. All 11 webui + 3 swipe tests + 14 workspace test binaries pass. Branch: `feat/webui-swipe-dual-terminal-2pf6`.
- 2026-04-25: **WebUI single-process dual binding** — `WebUiServer` now optionally binds a secondary plaintext HTTP listener in addition to the always-on `127.0.0.1:8484` and the optional TLS listener. Same router, same auth middleware, same TUI state. Eliminates the prior workaround of running two `orrchestrator` instances (one for `127.0.0.1:8484`, another for `0.0.0.0:80`) — that was producing divergent UIs because state is per-process. New env vars `ORRCH_WEBUI_PUBLIC_HTTP_PORT` and `ORRCH_WEBUI_PUBLIC_HTTP_BIND`. Esc menu surfaces the new URL row alongside local + TLS public. Branch: `feat/webui-dual-listener-public-http-9c2f`.
- 2026-04-22: **Instruction inbox refreshed — 3 new items (OPT-018 through OPT-020) routed from idea 2026-05-06-03-57 and incorporated as roadmap items 120-123** in new "UI Polish & Settings & Enterprise UX" section. Focus: truncate webportal URL display (OPT-018, cross-ref item 37); dedicated settings popup modal centralizing all user-configurable options (OPT-019, cross-ref items 59/OPT-005/OPT-013/TOK-002/OPT-011); TUI UX audit (OPT-020a) followed by enterprise-grade refactor via `frontend-design` skill (OPT-020b, cross-cuts every panel). OPT-020 split into audit-first + refactor-second sub-items. No structural conflicts with existing roadmap; OPT-020b builds on completed nav/scroll/keybind work rather than replacing it.
...
- 2026-04-13: **Oversee/project-detail continuation pass.** `crates/orrch-tui` now focuses newly spawned idea/editor windows via `wmctrl`, routes `→` from Oversee directly into project detail, exits project detail with `←`, removes the rendered dev-map pane from project detail, and renders the roadmap as a stateful list so selection/scroll stay aligned. Oversee now shows a `no-plan` indicator for planless projects and switches the hint line when `Project::roadmap_complete()` is true. Editor-facing wording was also aligned from `vim` to `nvim` across the TUI and entrypoint docs. Commits: `489f2ec`, `1ffb870`.
