...
## Dev Session: 2026-04-08 — Cap-Lifted Wave: Items 27b/c + 28 + 35 + 51 + 54 + 57 (7 clusters, Phase 5 unblocked)

### Completed
- **27b. Inbox maintenance walker** (`crates/orrch-core/src/feedback.rs`, `lib.rs`): Added `maintain_all_project_inboxes(projects_dir, max_bytes) -> Result<InboxMaintenanceReport>` and the `InboxMaintenanceReport { truncated: Vec<PathBuf>, trimmed: Vec<(PathBuf, usize)> }` struct. Walks subdirs of `projects_dir`, skips dotfiles + non-dirs + missing inbox files, calls both `truncate_inbox_if_large` and `trim_completed_entries` per project, logs per-project errors via `tracing::warn!` and continues. 1 test (`test_maintain_all_project_inboxes`) covering 4 cases. Both functions re-exported from `lib.rs`.
- **27c. Hypervisor handoff compression** (`crates/orrch-agents/src/runner.rs`): Added `compress_handoff(text: &str) -> String` (plain string ops, no `regex` dep): strips `<thinking>...</thinking>` blocks via state machine (multiline-safe, multi-block), drops case-insensitive preamble lines ("let me", "i'll start by", "looking at this", etc.) UNLESS inside a fenced code block (toggled on ```), collapses 3+ blank lines to 1. Wired into `build_handoff_prompt` so the previous-agent output is compressed before injection. 5 new tests covering all four behaviors plus existing `test_handoff_prompt` still passes.
- **28. Library storage backend — sync primitives** (`crates/orrch-library/src/sync.rs`, `lib.rs`, `Cargo.toml`, `crates/orrch-core/src/config.rs`): New `sync` module with `clone_if_missing(library_dir, repo_url) -> Result<()>` (no-op when `.git` already exists), `sync_pull(library_dir) -> Result<()>` (`git -C ... pull --ff-only`), `sync_push(library_dir, message) -> Result<()>` (`git add -A`, detect no-changes via `git diff --cached --quiet`, commit + push). All shell out via `std::process::Command` with `.arg("-C").arg(library_dir)`, error on `output.status.success() == false` with `bail!(stderr)`. `anyhow` added to orrch-library deps. `Config::library_repo_url: Option<String>` added with `#[serde(default)]`. 3 unit tests using temp dirs + `git init`. **This is the Phase 5 unblocker** — items 31, 32, 58 can now reference real library sync.
- **35. Nested workforce engine resolver** (`crates/orrch-workforce/src/engine.rs`, `lib.rs`): Added `NestedExpansion { inner_workforce, output_agent_id, output_agent_profile }` struct and `expand_nested_workforce(parent, all_workforces, node_id) -> Option<NestedExpansion>`. Lookup logic: find node in `parent.agents` by id, read `nested_workforce` field, find matching workforce in `all_workforces` by name, identify output agent (first `user_facing == true` else first agent overall), return expansion or `None`. 4 tests cover valid resolution, missing nested ref, missing inner workforce, fallback path.
- **51. Direct PM interaction** (`library/skills/pm-plan-edit.md`, `crates/orrch-tui/src/app.rs`): New skill file with YAML frontmatter (name/description/tags) + body defining the PM agent's plan-editor operating mode, constraints, and clean-exit behavior. Single `KeyCode::Char('P')` binding added to `key_project_detail` (~app.rs:2694) that calls the existing `spawn_session` helper with `BackendKind::Claude` and a goal invoking `/agent:project_manager` + `pm-plan-edit` skill. No new state fields, no new SubView, no conflict with the existing `'P'` binding in `key_projects` (different SubView).
- **54. Three-tier workforce templates** (`workforces/enterprise_tier.md`, `workforces/mid_tier.md`, `workforces/local_tier.md`): Three pure-markdown workforce files. Enterprise = 5 agents (PM/Dev/Engineer/Researcher/Feature Tester), loose connections, trusts judgment. Mid = 6 agents adding a Specialist reviewer with explicit dev → spec → PM report path. Local = 4 agents (PM/Dev/Feature Tester/Penetration Tester), narrow rigid scope with dual verification. All three use the existing 4-column `## Agents` table and parse via `parse_workforce_markdown` without parser changes.
- **57. Mixed-model `Step` field** (`crates/orrch-workforce/src/operation.rs`, `parser.rs`): Added `Step::model_override: Option<String>` with `#[serde(default, skip_serializing_if = "Option::is_none")]`. Step-table parser in `parse_operation_markdown` accepts optional 5th column "Model"; legacy 4-column tables still parse cleanly. Added `test_parse_step_with_model_override` and a legacy-compat test. Hypervisor runtime wiring (actually using the override during spawn) still pending. Cluster 6 also made minimal additive `model_override: None` edits to test literals in `engine.rs` and `runner.rs` to keep the workspace compiling.

### Verification
- **Tester A** (clusters 1, 2, 3 — orrch-library + orrch-core feedback + orrch-workforce engine): PASS, no real bugs. Confirmed file:line locations, error handling, dotfile-skip logic, fallback semantics, and all unit tests green.
- **Tester B** (clusters 4, 5, 6, 7 — orrch-agents + workforces md + orrch-workforce step + orrch-tui app): PASS, no real bugs. Confirmed `compress_handoff` wiring at runner.rs:184, tier files match `general_software_development.md` structure with correct agent profile names, parser at parser.rs:329 handles 5-column + legacy-4-column step tables, `'P'` binding at app.rs:2694 is in the correct subview without conflict.
- **Workspace verification**: `cargo build` clean, `cargo test --workspace` 210 tests pass across 7 crates (orrch-agents 18, orrch-core 97, orrch-library 21, orrch-workforce 25, orrch-retrospect 29, orrch-tui 5, orrch-mcp 15).
- **PM verdict**: SHIP. Both testers unanimous PASS, no rework cycles.

### Workflow Notes
- Workflow: `develop_feature` MCP dispatch → `workflow_init` → PM (1 spawn, cap LIFTED to 5-8) → `workflow_cluster` (7 clusters in 1 wave, all parallelizable) → 7 Developers spawned in 2 batches of 3+4 (cargo lock contention forced de facto serialization on the cargo build phase but the LLM work parallelized) → 2 Testers in parallel covering 7 clusters → PM evaluation inline → finalize.
- **Cap lift impact**: Last session = 2 items, this session = 7 items. ~3.5x throughput. The `workflow_init` filter fix (separate commit `15a7053`) plus a permissive PM prompt that allowed "new infrastructure" work made this possible.
- **Race condition handled**: Cluster 6 (mixed-model `Step` field) had to make minimal additive edits to `engine.rs` (Cluster 3's territory) and `runner.rs` (Cluster 4's territory) because adding a non-defaulted struct field forces every literal construction site to be updated. The race resolved cleanly because each cluster's edits were additive and non-overlapping at the line level — workspace compiles + all tests green.
- **Library sync is the Phase 5 unblocker**: items 31 (AI-assisted creation), 32 (Auto-assignment via Mentor), 58 (Mentor resource updates) all depend on a working library backend. The sync primitives in `crates/orrch-library/src/sync.rs` give them a real foundation to build on.
...
- **36. Workforce import/export** — Added the serializer side of the workforce markdown format so workforce designs can round-trip to disk and back. Three-cluster sequential implementation:
  ...
  - **Cluster 3** (`crates/orrch-tui/src/app.rs`, `ui.rs`): `x` key in Design > Workforce > Workflows tab exports the selected workforce to `~/Downloads/<sanitized_name>.md` (sanitizer: spaces → `_`, filter to ASCII alnum/`-`/`_`, fallback "workforce"). Uses existing `self.notify()` toast for success/failure. Creates `~/Downloads` if missing via `create_dir_all`. `i` key reads `~/Downloads/import.md` (Option B — hardcoded path, no file picker, private-scope-appropriate), parses, and pushes into both `workforce_files` (sorted) and `loaded_workforces` so it renders immediately. Full `orrch_workforce::engine::` path used (functions not re-exported from workforce `lib.rs`). Help line in `ui.rs` updated with `x=export i=import` on the Workflows tab title. HOME via `std::env::var("HOME")` with `/home/user` fallback, no new deps.
